...
{
    public class HibernateCountryAdditionalHourDao : HibernateDao<CountryAdditionalHour>, ICountryAdditionalHourDao
    {
                                                                      ...
                                                                      bool bForReading, out string filterHQL, User user)
        {
            ...
            sFrom.AppendFormat("SELECT filtered.ID FROM {0} filtered", typeof(CountryAdditionalHour).Name);
        ...
        public List<CountryAdditionalHour> GetCountryAdditionalHourFiltered(long CountryID, short? Year)
        {
            ...
            StringBuilder whereStr = new StringBuilder(" entity.CountryID = :country_id ");
            pNames.Add("country_id");
            pValues.Add(CountryID);
            
            if(Year.HasValue)
            {
                whereStr.Append(" AND entity.Year = :year ");
                pNames.Add("year");
                pValues.Add(Year.Value);
            }

            return GetTypedListFromIList(FindByNamedParam(whereStr.ToString(), pNames.ToArray(), pValues.ToArray()));
        }
    }
}