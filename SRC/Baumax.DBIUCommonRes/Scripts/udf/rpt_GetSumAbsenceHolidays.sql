print '%Creating function% [rpt_GetSumAbsenceHolidays]'
go
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[rpt_GetSumAbsenceHolidays]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[rpt_GetSumAbsenceHolidays]
GO
...
-- obsolete now
/*
-- Sum of absences with holidays
CREATE FUNCTION [dbo].[rpt_GetSumAbsenceHolidays] (@ReportName nvarchar(30), @EmployeeID bigint, @YearNum int, @MonthNum int, @IncludeWeekends bit, @IncludeFeasts bit)
RETURNS bigint
