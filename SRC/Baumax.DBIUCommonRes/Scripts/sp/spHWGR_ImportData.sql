...
begin
	...
	begin try
		...
		set @ImportDate = getdate()--convert(datetime,convert(varchar(12),getdate(), 112),112)
	--Add data to vwHWGR	
	...
	--add data to World_HWGR
		...
		insert into dbo.World_HWGR (World_HWGR_ID, Store_WorldID, HWGR_ID, BeginTime, EndTime, Import)
		select 
		...
		where not exists (select * from World_HWGR where World_HWGR.HWGR_ID = hw.HWGR_ID and World_HWGR.Store_WorldID = sw.Store_WorldID and World_HWGR.Import = 1 and World_HWGR.EndTime > @ImportDate)
		set @ID = @ID + @@rowcount
