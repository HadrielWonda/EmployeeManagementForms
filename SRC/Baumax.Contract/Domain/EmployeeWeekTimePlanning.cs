...
namespace Baumax.Domain
{
    ...
    #region EmployeeWeekTimePlanning

    /// <summary>
    /// EmployeeWeekTimePlanning object for NHibernate mapped table 'EmployeeWeekTimePlanning'.
    /// </summary>
    [Serializable]
    public class EmployeeWeekTimePlanning : BaseEntity, IEmployeeWeek
    {
        ...
        public EmployeeWeekTimePlanning()
        {
        }

        public EmployeeWeekTimePlanning(byte weekNumber, DateTime weekBegin, DateTime weekEnd, int plusMinusHours,
                                        int saldo, int plannedHours, int contractHours, int additionalCharge,
                                        long employeeID, int workingHours, bool customEdit, long orderHWGR)
        {
        ...
        public virtual string Dump()
        {
            return String.Format(@" ID={0} Week={1} From={2} To={3}\n 
                                   PlannedHours={4} AddCharges={5} WorkingHours={6} ContractHours={7}\n
                                    Saldo={8} AllIn={9} CustomEdit={10}", new object[] { EmployeeID, WeekNumber, WeekBegin.ToShortDateString(),
                WeekEnd.ToShortDateString(), PlannedHours, AdditionalCharge, WorkingHours, ContractHours, Saldo, AllIn, CustomEdit});
        }
    }
